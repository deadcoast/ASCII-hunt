...
class ImportLocalityOptimizer:
    ...
    def _recommend_placement(self, import_group, usage_map):
        ...
        if avg_first_usage < 50:
            return "top"
        elif avg_first_usage < 200:
            return "middle_top"
        elif avg_first_usage < 500:
            return "middle"
        else:
            return "bottom"
    ...
    def _explain_grouping(self, import_group, usage_map):
                    ...
                    usage_patterns.append(
                        {
                            ...
                            "span": max(usage_lines) - min(usage_lines)
        ...
        if len(modules) == 1:
            ...
            if usage_patterns and len(usage_patterns) > 1:
                regions = {}
                ...
                if len(regions) == 1:
                    ...
                    region_counts = [
                        (region, count) for region, count in regions.items()
                    ]
                    if region_counts:
...
class SymbolicImportOptimizer:
    ...
    def build_symbolic_model(self):
        ...
        try:
            ...
            return {sym: 1.0 for sym in self.symbol_table.values()}

...
def create_dynamic_box(content, title=None, padding=2, min_width=None, max_width=None, center=True):
    """
    ...
    for line in content:
        ...
        if display_width > box_width - (padding * 2) - 2:
            ...
            i = 0
            while i < len(line_without_ansi) and current_width < max_content_width - 3:  # -3 for "..."
            ...
            for char in truncated:
                while original_idx < len(line) and line[original_idx] != char:
                    if line[original_idx] == '\033':
                        ...
                        ansi_seq = line[original_idx:original_idx+re.search(r'm', line[original_idx:]).end()]
...
def analyze_string(text):
    ...
    # Detect patterns
    results['has_box_drawing'] = any(c in '─│┌┐└┘├┤┬┴┼╌╍╎╏═║╒╓╔╕╖╗╘╙╚╛╜╝╞╟╠╡╢╣╤╥╦╧╨╩╪╫╬╱╲╳╴╵╶╷╸╹╺╻╼╽╾╿' for c in text)
    results['has_wide_chars'] = any(unicodedata.east_asian_width(c) in ['F', 'W'] for c in text)
    results['has_zero_width'] = any(unicodedata.category(c) in ['Mn', 'Me', 'Cf'] for c in text)
...
def format_analysis_results(analysis):
    ...
    lines.append(f"• Character count: {COLORS['green']}{analysis['char_count']}{COLORS['reset']}")
    lines.append(f"• Display width: {COLORS['green']}{analysis['display_width']}{COLORS['reset']}")
    lines.append(f"• UTF-8 bytes: {COLORS['green']}{analysis['bytes']}{COLORS['reset']}")
    ...
    if analysis['char_count'] != analysis['display_width']:
        ...
        if analysis['display_width'] > analysis['char_count']:
            ...
            lines.append(f"{COLORS['yellow']}Width exceeds count by {diff} columns{COLORS['reset']}")
            ...
            lines.append(f"{COLORS['yellow']}Count exceeds width by {diff} characters{COLORS['reset']}")
    ...
    if analysis['char_types']:
        ...
        for char_type, count in sorted(analysis['char_types'].items(), key=lambda x: x[1], reverse=True):
    ...
    if analysis['has_box_drawing']:
        lines.append("")
        lines.append(f"{COLORS['cyan']}Contains box-drawing characters{COLORS['reset']}")
        ...
        try:
            ...
            if user_input.lower() in ['exit', 'quit']:
                ...
                if CLIPBOARD_AVAILABLE:
                    ...
                    print(f"{COLORS['yellow']}Run: pip install pyperclip{COLORS['reset']}")
                    continue
